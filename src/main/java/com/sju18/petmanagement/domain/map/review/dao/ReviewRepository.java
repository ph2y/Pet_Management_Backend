...
public interface ReviewRepository extends JpaRepository<Review, Long> {
    ...
    Page<Review> findAllByPlaceId(@Param("placeId") Long placeId, Pageable pageable);

    @Query(
            value = "SELECT * FROM review AS r WHERE r.review_id <= :top AND r.place_id=:placeId",
            countQuery = "SELECT COUNT(*) FROM review AS r WHERE r.review_id <= :top AND r.place_id=:placeId",
            nativeQuery = true
    )
